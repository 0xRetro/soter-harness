...
description: >-
  Captures an organization as a [DB] Orgs row — following the shared write-discipline,
  with Type classified from sector prose (sector words go to Tags, not Type), handles
  normalized to URLs, and dedup stressed because orgs are relation targets. Use when the
  user wants to add, capture, or log an organization or company into the CRM. Not for
  people (that is /capturing-a-contact), not for the Notion write mechanics
  (/pushing-to-notion), and not for projects.
