...
assert.equal(
  ...
  false,
  'CRM must not retain the removed pre-cutover Channel compatibility shim'
);
...
for (const legacyPath of [
  ...
  assert(!claudeProjection.outputs.some((output) => output.path === legacyPath));
}
assert(claudeAdapter.limitations.some((value) => {
  return value.includes('candidate guides and retained legacy skills remain excluded');
}));
assert(claudeProjection.limitations.some((value) => {
  return value.includes('candidate and legacy Claude skills')
...
assert(!developmentCatalogLock.projections.some((output) => output.path.startsWith('.claude/skills/')),
  'development catalog must not realize candidate or retained legacy Claude skills');
...
assert(!claudeProjectionLock.projections.some((output) => output.path.startsWith('.claude/skills/')),
  'static Claude host projection must not realize candidate or retained legacy Claude skills');
...
const migration = load('soter/migrations/legacy-foundations.migration.json');
const legacyInventory = load('soter/migrations/legacy-inventory.json');
for (const item of migration.items) {
  assert(['migrated', 'retired'].includes(item.state));
  assert.equal(item.evidence.length, 1);
  const source = legacyInventory.items.find((candidate) => {
    return candidate.sourcePath === item.sourcePath;
  });
  const binding = source?.targets.find((candidate) => {
    return candidate.id === item.targetPack && candidate.path === item.targetPath;
  });
  assert(source, `${item.sourcePath} source tombstone is absent from inventory`);
  assert(binding, `${item.sourcePath} final responsibility is absent from inventory`);
  assert.equal(
    item.sourceFingerprint,
    source.sourceFingerprint,
    `${item.sourcePath} migration must bind the exact removed source fingerprint`
  );
  assert(fs.existsSync(path.join(root, item.targetPath)), `${item.targetPath} does not exist`);
  assert.equal(fs.existsSync(path.join(root, item.sourcePath)), false);
  assert.equal(source.sourcePresence, 'removed');
  assert(['migrated', 'retired'].includes(source.state));
  assert.equal(binding.state, item.state);
  assert.equal(binding.canonicalAuthority, item.state === 'migrated' ? 'target' : 'none');
  assert.equal(binding.fallback, 'removed');
  assert(['proven', 'intentional-change'].includes(binding.parity));
  assert.deepEqual(binding.evidence, item.evidence);
}

const productSourcePath = '.claude/systems/product-development.md';
const productSource = legacyInventory.items.find((item) => item.sourcePath === productSourcePath);
const productBinding = productSource?.targets.find((target) => {
  return target.id === 'context.product'
    && target.path === 'soter/packs/context.product/pack.json';
});
assert(productSource, 'Product legacy source must remain as one exact inventory tombstone');
assert(productBinding, 'Product Context migration binding must remain exact');
assert.equal(fs.existsSync(path.join(root, productSourcePath)), false);
assert.equal(productSource.sourcePresence, 'removed');
assert(['migrated', 'retired'].includes(productSource.state));
assert.equal(productBinding.state, 'migrated');
assert.equal(productBinding.canonicalAuthority, 'target');
assert.equal(productBinding.fallback, 'removed');
assert(['proven', 'intentional-change'].includes(productBinding.parity));
assert(productSource.targets.every((target) => {
  return ['migrated', 'retired'].includes(target.state)
    && target.fallback === 'removed';
}));
assert(productSource.retirementCriteria.includes('deliberately retire'));

const serialized = JSON.stringify({
  policy,
  contextModels: [...contextModels.values()],
  contextVocabularies: [...governedVocabularies.values()],
  claudeAdapter,
  claudeProjection
});
assert(!/(?:sk|pk)_(?:live|test)_[A-Za-z0-9]+/.test(serialized));
assert(!/(?:^|[\s"'])\/(?:[^/\s"']+\/)*[^/\s"']+/.test(serialized));

process.stdout.write('Legacy foundations selftest passed: exact policy structure, bounded Context ownership, credential exclusions, governed Context vocabularies, request-scoped development effects, effect-free static host projection, active-guide-only Claude projection, and exact migration fingerprints remain enforced.\n');
