...
function checkPackGraph(root, documents, out, census) {
  ...
  for (const entry of documents) {
    if (entry.contractId === 'soter://contracts/pack/v1') {
    ...
    } else if (entry.contractId === 'soter://contracts/host-projection-definition/v1') {
      ...
      || entry.contractId === 'soter://contracts/evidence/v2') {
      census.evidence += 1;
  ...
  for (const [id, entry] of packs) {
    ...
    if (entry.doc.operator) {
      ...
      if (entry.doc.operator.connection) {
        const implementation = entry.doc.artifacts.find((artifact) => {
          return artifact.path === entry.doc.operator.connection.module
            && artifact.role === 'implementation';
        });
        if (!implementation) {
          out.push(violation(
            ...
            'connected compiler is not a declared implementation artifact: '
              + entry.doc.operator.connection.module,
    ...
    if (lock) {
      ...
      if (entry.contractId === 'soter://contracts/evidence/v2') {
        ...
        const observedAuthorities = [...entry.doc.authorities]
          .sort((left, right) => left.id.localeCompare(right.id));
        if (entry.doc.graphFingerprint !== lock.doc.graphFingerprint
          ...
          out.push(violation(
            ...
            'regenerate the evidence from the exact current lock rather than copying or editing applicability fields'
          ));
        }
      }
    }
...
function selftest(root) {
  ...
  try {
    ...
    const originalDecisionText = fs.readFileSync(decisionFile, 'utf8');
    const invalidConditionalDecision = JSON.parse(originalDecisionText);
    ...
    fs.writeFileSync(decisionFile, originalDecisionText);
    ...
    invalidPolicyFixture.data.records
      .find((record) => record.id === 'policy.project-pulse')
      .fields.healthMustBeExplained = 'yes';
    ...
    if (!badPolicyFixture.violations.some((item) => item.code === 'SOTER_PROVIDER_FIXTURE_CONTEXT')) {
      failures.push('planted invalid project-status policy field was not detected');
    }
    const invalidMilestoneFixture = JSON.parse(originalProviderFixtureText);
    invalidMilestoneFixture.data.records
      .find((record) => record.type === 'milestone')
      .fields.workItemsTotal = 'many';
    fs.writeFileSync(providerFixtureFile, JSON.stringify(invalidMilestoneFixture, null, 2) + '\n');
    const badMilestoneFixture = verifySoter(temp);
    if (!badMilestoneFixture.violations.some((item) => item.code === 'SOTER_PROVIDER_FIXTURE_CONTEXT')) {
      failures.push('planted invalid milestone progress field was not detected');
    }
    const invalidDateFixture = JSON.parse(originalProviderFixtureText);
    invalidDateFixture.data.records
      .find((record) => record.id === 'soter-fixture://crm/task/existing-deck')
  ...
  console.log('SELFTEST PASS: schema vocabulary, composition, conditionals, bounds, deep uniqueness, version, clean graph, prepared-work and proposal ownership, pack settings, portable sources, Context record model, provider mapping, native host tool, binding, host, lock host selection, exact evidence applicability, malformed JSON, unknown-contract, and malformed-contract checks fired as expected.');
  return true;
}
