...
function reidentifySelection(selection, work) {
  const identity = {
    workId: work.id,
  ...
  selection.id = 'review-only-candidate-selection.' + work.automation.id.slice('automation.'.length) + '.' + hex;
  return resignSelection(selection);
}
...
function createSelectionProcess(root, workId, actionId) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [
      'soter/core/cli.mjs',
      'operator-review-only-candidate-selection-create',
      '--work-id', workId,
      '--action-id', actionId,
      '--json'
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        reject(new Error('Concurrent selection create failed: ' + stderr));
        return;
      }
      try {
        resolve(JSON.parse(stdout));
      } catch (error) {
        reject(new Error('Concurrent selection create returned invalid JSON.', { cause: error }));
      }
    });
  });
}

export async function selftestReviewOnlyCandidateSelections(root = defaultRoot) {
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'soter-review-only-candidate-selection-selftest-'));
  try {
    ...
    const focus = 'PRIVATE_REVIEW_ONLY_CANDIDATE_SELECTION_FOCUS_SENTINEL';
    ...
    assert.equal(selection.$contract, 'soter://contracts/review-only-candidate-selection/v1');
    assert.equal(selection.state, 'review-only');
    assert.equal(selection.scope.availableActionCount, proposed.length);
    assert.equal(selection.scope.selectedActionCount, 2);
    assert.equal(selection.scope.partial, true);
    assert.deepEqual(selection.actions.map((action) => action.id), [label.id, draft.id],
      'Selection order must follow the immutable prepared review, not request order.');
    assert.deepEqual(selection.effects, ['write']);
    ...
    assert.equal(selection.privacy.approvalAuthorityIncluded, false);
    assert.equal(selection.privacy.continuationAuthorityIncluded, false);
    assert.equal(selection.privacy.executionAuthorityIncluded, false);
    const serializedSelection = JSON.stringify(selection);
    assert(!serializedSelection.includes('in:inbox newer_than:1d'));
    assert(!serializedSelection.includes(focus));
    assert(!serializedSelection.includes('Complete draft body'));
    assert(!serializedSelection.includes('Thanks for the note.'));

    const selectionFile = reviewOnlyCandidateSelectionStatePath(temporaryRoot, selection.id);
    assert.equal(fs.statSync(selectionFile).mode & 0o777, 0o600);
    assert.equal(fs.statSync(path.dirname(selectionFile)).mode & 0o777, 0o700);
    const reentered = createReviewOnlyCandidateSelection({
      root: temporaryRoot,
      workId: work.id,
      actionIds: [label.id, draft.id],
      createdAt: '2026-07-16T18:02:00.000Z'
    });
    assert.equal(reentered.fingerprint, selection.fingerprint);
    assert.equal(reentered.createdAt, selection.createdAt,
      'Exact re-entry must not replace an immutable candidate selection.');

    const concurrent = await Promise.all([
      createSelectionProcess(temporaryRoot, work.id, draft.id),
      createSelectionProcess(temporaryRoot, work.id, draft.id),
      createSelectionProcess(temporaryRoot, work.id, draft.id),
      createSelectionProcess(temporaryRoot, work.id, draft.id)
    ]);
    ...
    assert.equal(material.$contract, 'soter://contracts/review-only-candidate-selection-material/v1');
    assert.equal(material.selection.fingerprint, selection.fingerprint);
    ...
    assert.throws(() => createReviewOnlyCandidateSelection({
      root: temporaryRoot,
      workId: work.id,
      actionIds: [label.id, label.id]
    }), (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_INVALID');
    ...
    assert.throws(() => createReviewOnlyCandidateSelection({
      ...
      return error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_INVALID'
    ...
    const substitutedSelection = structuredClone(selection);
    substitutedSelection.actions[0].proposedValueFingerprint = selection.actions[1].proposedValueFingerprint;
    substitutedSelection.scope.fingerprint = fingerprintJson(substitutedSelection.actions);
    resignSelection(substitutedSelection);
    assert.throws(
      () => assertReviewOnlyCandidateSelection(temporaryRoot, substitutedSelection, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_BINDING_INVALID'
    );
    const substitutedSelectionBasis = structuredClone(selection);
    substitutedSelectionBasis.configuration.configurationBasis = 'private-active';
    resignSelection(substitutedSelectionBasis);
    assert.throws(
      () => assertReviewOnlyCandidateSelection(temporaryRoot, substitutedSelectionBasis, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_BINDING_INVALID'
    );
    const reorderedSelection = structuredClone(selection);
    reorderedSelection.actions.reverse();
    reorderedSelection.actions.forEach((action, index) => {
      action.sequence = index + 1;
    });
    reorderedSelection.scope.fingerprint = fingerprintJson(reorderedSelection.actions);
    reidentifySelection(reorderedSelection, work);
    assert.throws(
      () => assertReviewOnlyCandidateSelection(temporaryRoot, reorderedSelection, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_BINDING_INVALID'
    );
    ...
    assert.throws(
      () => assertReviewOnlyCandidateSelectionMaterial(temporaryRoot, substitutedMaterial, selection, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MATERIAL_BINDING_INVALID'
    );
    ...
    assert.throws(
      ...
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MATERIAL_TAMPERED'
    );
    ...
    assert.throws(
      () => assertReviewOnlyCandidateSelectionMaterial(temporaryRoot, falseApplicability, selection, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MATERIAL_BINDING_INVALID'
    );
    ...
    assert.throws(
      () => assertReviewOnlyCandidateSelectionMaterial(temporaryRoot, rawProviderEscape, selection, work),
      (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MATERIAL_MALFORMED'
    );

    const taskTitle = 'PRIVATE_TASK_CANDIDATE_TITLE_SENTINEL';
    ...
    assert.equal(taskSelection.scope.availableActionCount, 1);
    assert.equal(taskSelection.scope.selectedActionCount, 1);
    assert.equal(taskSelection.scope.partial, false);
    assert.deepEqual(taskSelection.blockers, [
      'REVIEW_ONLY_CANDIDATE_PREVIEW_NOT_CREATED',
      'CONNECTED_VERIFICATION_NOT_PROVEN'
    ]);
    assert(!JSON.stringify(taskSelection).includes(taskTitle));
    assert(!JSON.stringify(taskSelection).includes(taskDate));
    const taskMaterial = inspectReviewOnlyCandidateSelectionMaterial({
      root: temporaryRoot,
    ...
    assert(!serializedWorkspace.includes(selection.id));
    assert(!serializedWorkspace.includes(taskSelection.id));
    ...
    assert.equal(fingerprintPath(path.join(temporaryRoot, 'soter')), canonicalBefore,
      'Review-only candidate selection work changed canonical artifacts.');
    ...
    const staleMaterial = inspectReviewOnlyCandidateSelectionMaterial({
      root: temporaryRoot,
      selectionId: selection.id
    });
    ...
    assert.throws(() => createReviewOnlyCandidateSelection({
      ...
      workId: work.id,
      actionIds: [label.id]
    }), (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_STALE');

    assert.throws(() => inspectReviewOnlyCandidateSelectionMaterial({
      root: temporaryRoot,
      selectionId: 'review-only-candidate-selection.missing.value'
    }), (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MISSING');
    fs.writeFileSync(selectionFile, '{ malformed private state\n');
    assert.throws(() => inspectReviewOnlyCandidateSelectionMaterial({
      root: temporaryRoot,
      selectionId: selection.id
    }), (error) => error.code === 'REVIEW_ONLY_CANDIDATE_SELECTION_MALFORMED');
    process.stdout.write(
      'REVIEW-ONLY CANDIDATE SELECTION SELFTEST PASS: exact partial subsets, create-only private state, '
